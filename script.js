...
(function () {
    ...
    /* ──────────────────────────────────────────────────────────
       SECCIÓN 1 — CONSTANTES GLOBALES
       ──────────────────────────────────────────────────────────
       IMG_FOLDER : carpeta donde están las imágenes
       IMG_EXT    : extensión de las imágenes
    ...
    function imgPath(n) {
        return `${IMG_FOLDER}/${String(n).padStart(5, '0')}.${IMG_EXT}`;
    }

    /** Comprueba si la imagen n existe (resuelve true/false) */
    ...
    /**
     * Detecta cuántas imágenes hay usando búsqueda binaria.
     * 1. Dobla el índice hasta encontrar un "hueco" (límite superior).
     * 2. Bisecta entre lo y hi para encontrar el último válido.
     * Llama a callback(count) cuando termina.
     */
    function detectImageCount(callback) {
        let lo = 0, hi = 1;

        /* Fase 1: encontrar límite superior */
        function findUpper() {
            imageExists(hi).then(found => {
                if (found) { lo = hi; hi *= 2; findUpper(); }
                else { binarySearch(); }
            });
        }

        /* Fase 2: búsqueda binaria entre lo y hi */
        function binarySearch() {
            if (hi - lo <= 1) { callback(lo); return; }
            const mid = Math.floor((lo + hi) / 2);
            imageExists(mid).then(found => {
    ...
    /* ── Iniciar todo el sistema una vez que sepamos cuántas imágenes hay ── */
    detectImageCount(function (IMG_COUNT) {
        if (IMG_COUNT === 0) { console.warn('No se encontraron imágenes en', IMG_FOLDER); return; }

        const IMG_FILES = Array.from({ length: IMG_COUNT }, (_, i) => imgPath(i + 1));
        const TOTAL = IMG_FILES.length;
        const HERO_END = TOTAL * PX_STEP;

        /* Actualizar la altura del scroll-stage para que coincida con el total real */
        const stage = document.getElementById('scroll-stage');
        if (stage) stage.style.height = `calc(100vh + ${TOTAL * PX_STEP}px)`;
        ...
        camera.position.set(0, -5, 80);   /* cámara alejada 80 unidades en Z, desplazada -5 en Y */
